...
const { data: loginUrlResponse, error: loginUrlError } = await useFetch<Record<string,string> | null>('https://musikk.localhost/api/spotify/login-url', {
    method: 'GET',
    headers: {
...
const fistLogin = async () => {
    console.log(code.value);
    const { data: tokenResponse, error: tokenError } = await useFetch<Record<string,string>>(`https://musikk.localhost/api/spotify/first-login-workflow/${code.value}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${user?.value?.token}`,
